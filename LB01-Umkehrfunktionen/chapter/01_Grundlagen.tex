...
\begin{note}
    \raisebox{-\height}{
        \begin{tikzpicture}[
                declare function={
                    ...
                    f_f(\x) = (0.5*\x*\xScale + 1)*(1/\yScale);
                    f_f1(\x) = (2.0*\x*\xScale - 2)*(1/\yScale);
                }
            ]
            ...
            \addplot[thick, myb, samples=1000, domain=\xLowerLimitGrid:0.96*\xUpperLimitGrid]
                {f_f(x)};
            \node[anchor=north] at (axis cs:{5*(1/\xScale)},{3*(1/\yScale)}) {\textcolor{myb}{$G_{f}$}};

            % G_f^{-1}
            \addplot[thick, myr, samples=1000, domain=\xLowerLimitGrid:0.96*\xUpperLimitGrid]
                {f_f1(x)};
            \node[anchor=north] at (axis cs:{3*(1/\xScale)},{6*(1/\yScale)}) {\textcolor{myr}{$G_{f^{-1}}$}};
    ...
    \begin{itemize}
        \item $G_{f^{-1}}$ entsteht aus $G_f$ durch Spiegelung an der \texthl{\textbf{Winkelhalbierenden}} \\ des \Rom{1} und \Rom{3} Qudranten.
        \item $\highlight[yellow]{D_{f^{-1}} = W_{f}}$ \qquad und \qquad $\highlight[yellow]{W_{f^{-1}} = D_{f}}$
        \item \begin{minipage}[t]{0.65\linewidth}
            \raisebox{-\height}{
                \begin{tikzpicture}[
                        declare function={
                            f_g(\x) = ((\x*\xScale)^2)*(1/\yScale);
                            f_g1(\x) = (sqrt(\x*\xScale))*(1/\yScale);
                            f_g2(\x) = (-sqrt(\x*\xScale))*(1/\yScale);
                        }
                    ]
                    ...
                    \addplot[thick, myb, samples=1000, domain=\xLowerLimitGrid:0.96*\xUpperLimitGrid]
                        {f_g(x)};
                    \node[anchor=north] at (axis cs:{2*(1/\xScale)},{3*(1/\yScale)}) {\textcolor{myb}{$G_{g}$}};
                    ...
                    \addplot[thick, myr, samples=1000, domain=0:0.96*\xUpperLimitGrid]
                        {f_g2(x)};
                    \node[anchor=north] at (axis cs:{3*(1/\xScale)},{1.5*(1/\yScale)}) {\textcolor{myr}{$G_{g^{-1}}$}};
                    \end{axis}
                \end{tikzpicture}
            }\\[2.0ex]
            \end{minipage}
            \begin{minipage}[t]{0.3\linewidth}
                \vspace{2.5cm}
                $g$ ist auf $\left] \ -\infty; 0 \ \right]$ umkehrbar sowie auf $\left[ \ 0; +\infty \ \right[$
            \end{minipage}\\
            ...
            \end{tightcenter}
            \vspace{-0.5cm}
            Eine Funktion ist genau dann umkehrbar, falls sie \texthl{\textbf{streng monoton}} (zunehmend oder abnehmend) ist.
        \item es gilt: \\{
            \vspace{-2.0ex}
            \begin{align*}
                f(f^{-1}(x)) &= x \\[1.0ex]
